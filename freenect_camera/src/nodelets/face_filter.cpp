...
namespace freenect_camera {
		...
		string FaceFilter::GenerateTempFilePath()
		{
			...
			string fileName = str(boost::format("/tmp/kinect-%04d-%02d-%02d--%02d-%02d-%02d--%05d.csv") % 
				(now->tm_year + 1900) % (now->tm_mon + 1) % now->tm_mday % now->tm_hour % now->tm_min % now->tm_sec % (counter ++) ); 
